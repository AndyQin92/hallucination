...
############################################# wsj add ############################################### 
def test_net_with_gt_boxes_without_eval(net, imdb, dist_file):
    num_images = imdb.img_nums
    object_pad_num = imdb.object_num
    feature_dim = 2048
    img_ids = imdb.img_ids()

    roidb = imdb.gt_roidb()
    h5f = h5py.File(dist_file, 'w')
    dataset = h5f.create_dataset("data",(num_images, object_pad_num, feature_dim),
                                     maxshape=(None, object_pad_num, feature_dim),
                                     dtype='float32')

    print("image num:",num_images)    
    for i in xrange(num_images):
        assert i==img_ids[i]
        feature = np.zeros((object_pad_num, feature_dim), dtype=np.float)
        box_proposals = roidb[i]['boxes']
        im = cv2.imread(imdb.image_path_at(i))
        pool5 = get_features_from_im(net, im, box_proposals, force_boxes=True)
        obj_num = len(pool5)
        feature[:obj_num] = pool5
        dataset[i] = feature
        if(i%1000==0):
            print("porcessed %d/%d img"%(i, num_images))

        # if i%3000 == 0:
        #     h5f.close()
        #     h5f = h5py.File(dist_file, 'a')
        #     dataset = h5f['data']
    h5f.close()

def test_net_with_gt_union_boxes_without_eval(net, imdb, dist_file):
    num_images = imdb.img_nums
    rel_pad_num = imdb.predicate_num
    feature_dim = 2048
    img_ids = imdb.img_ids()

    roidb = imdb.gt_reldb()
    h5f = h5py.File(dist_file, 'w')
    dataset = h5f.create_dataset("data",(num_images, rel_pad_num, feature_dim),
                                    maxshape=(None, rel_pad_num, feature_dim),
                                    dtype='float32')

    print("image num:",num_images)    
    for i in xrange(num_images):
        assert i==img_ids[i]
        feature = np.zeros((rel_pad_num, feature_dim), dtype=np.float)
        box_proposals = roidb[i]['boxes']
        im = cv2.imread(imdb.image_path_at(i))
        pool5 = get_features_from_im(net, im, box_proposals, force_boxes=True)
        rel_num = len(pool5)
        feature[:rel_num] = pool5
        dataset[i] = feature
        if(i%1000==0):
            print("porcessed %d/%d img"%(i, num_images))
...
def get_features_from_im(net, im, boxes=None, force_boxes=True):
