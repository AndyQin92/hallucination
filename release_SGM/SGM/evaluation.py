...
# Sijin Wang, Ruiping Wang, Ziwei yao, Shiguang Shan and Xilin Chen
# https://arxiv.org/abs/1910.05134
#
# Writen by Sijin Wang, 2019
# ---------------------------------------------------------------
"""Evaluation"""
...
import torch
from model import SGM, xattn_score_t2i, xattn_score_i2t
...
def encode_data_with_predicate(model, data_loader, log_step=10, logging=print):
    ...
    for i, (images, obj_num, captions, lengths, ids, image_predicates, pred_num, 
            rels, objs, cap_rels, cap_rel_num) in enumerate(data_loader):
    ...
    for i, (images, obj_num, captions, lengths, ids, image_predicates, pred_num, 
        ...
        img_emb, cap_emb, cap_len, pred_emb, cap_rel_emb, cap_rel_num = \
                   model.forward_emb(images, obj_num, captions, lengths, image_predicates, 
                                    pred_num, rels, objs, cap_rels, cap_rel_num)
        ...
        if img_embs is None:
            ...
            cap_rel_embs = np.zeros((len(data_loader.dataset), max_n_cap_rel, cap_rel_emb.size(2)))
        ...
        cap_rel_embs[ids, :max(cap_rel_num), :] = cap_rel_emb.data.cpu().numpy().copy()
        ...
        model.forward_loss(img_emb, obj_num, cap_emb, cap_len, pred_emb, pred_num, cap_rel_emb, cap_rel_num)
    ...
    return img_embs, obj_nums, cap_embs, cap_lens, pred_embs, pred_nums, cap_rel_embs, cap_rel_nums
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    data_loader = get_test_loader(split, opt.data_name, test_file_list,  opt.feature_name,  opt.vocab,
                                  opt.batch_size, opt.workers, opt)
    
    with torch.no_grad():
        print('Computing results...')
        img_embs, obj_nums, cap_embs, cap_lens, pred_embs, pred_nums,cap_rel_embs, cap_rel_nums \
        ...
        if not fold5:
            ...
            for i in range(5):
                ...
                cap_rel_embs_shard = cap_rel_embs[i * 5000:(i + 1) * 5000]
                ...
                if opt.cross_attn == 't2i':
                    ...
                    sims2 = shard_xattn_t2i(pred_embs_shard, pred_nums_shard, cap_rel_embs_shard, cap_rel_nums_shard, opt, shard_size=128)
                    sims = sims1 + opt.predicate_score_rate * sims2

                elif opt.cross_attn == 'i2t':
                    sims1 = shard_xattn_i2t(img_embs_shard, obj_nums_shard, cap_embs_shard, cap_lens_shard, opt, shard_size=128)
                    sims2 = shard_xattn_i2t(pred_embs_shard, pred_nums_shard, cap_rel_embs_shard, cap_rel_nums_shard, opt, shard_size=128)
                    sims = sims1 + opt.predicate_score_rate * sims2      
                else:    
                    raise NotImplementedError
